...
const RestaurantDetails = ({ navigation, route }) => {
    ...
    useEffect(() => {
        ...
        const getMenu = async () => {
            const response = await axios.get(`http://localhost:4000/item/${restaurant._id}/menu`, {
                headers: {
                    'Content-Type': 'application/json',
                },
                withCredentials: true,
        ...
        <View className='flex-1'>
            ...
            <View className=' flex flex-row justify-between items-center px-2 py-1'>
                ...
                <View className=' flex flex-row justify-center items-center'>
                    <Icon
                        ...
                        className="w-5 h-5"
                    />
                    <Text className='w-auto text-xs'>Customer's Rating : {restaurant.rating}</Text>
                </View>
            ...
            <View>
                ...
                <TouchableOpacity onPress={() => navigation.navigate('cart')}>
                    <Text className='text-center font-bold text bg-red-500 text-white w-24 mx-auto  rounded-md'>Checkout</Text>
                </TouchableOpacity>
            </View>
