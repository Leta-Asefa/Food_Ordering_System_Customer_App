...
const RestaurantListCard = ({navigation, item}) => {
  ...
  return (
    <View className="flex flex-row justify-between items-center mx-2 mb-1   bg-gray-100 rounded-xl">
      ...
      <View className="ml-4">
        <Text className="text-sm font-bold w-36 underline" numberOfLines={1}>
          {item.restaurant.name}
        </Text>
        <Text className="text-green-600 font-bold text-xs">
          {item.restaurant.opened ? 'opened' : 'closed'}
        </Text>
        ...
        <View className="flex flex-row items-center">
          ...
          <Text className="text-xs pl-1">
            {String(item.restaurant.rating).slice(0, 4)}
          </Text>
        </View>
        <View className="flex flex-row items-center">
      ...
      <View className="space-y-2 mr-1 flex-col justify-center  ">
        <View className="w-24">
          <Text numberOfLines={1} className="text-xs font-semibold text-right">
            {' '}
        ...
        <TouchableOpacity
          onPress={() => handleCall(item.restaurant.contact)}
          className="bg-green-600  px-2 py-0.5  flex-row  justify-center items-center  rounded-lg">
        ...
        <TouchableOpacity
          onPress={() => {
            if (item.restaurant.opened === false) {
          ...
          className={`${
            item.restaurant.opened ? 'bg-red-600' : 'bg-red-200'
          }  px-1 py-0.5  flex-row justify-center items-center  rounded-lg`}>
          <Ionicons name="list" size={10} color="#fff" />
          <Text className="text-white font-bold text-center text-xs px-1">
            Order{' '}
          </Text>
        </TouchableOpacity>
      </View>
