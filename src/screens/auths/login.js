...
const button =
  'w-36 text-xl text-center text-white bg-orange-600 rounded-lg px-3 py-1 mt-3 mx-auto';
  ...
  const handleSubmit = async () => {
      ...
      try {
        ...
        if (response.data.user?._id) {
          ...
          ToastAndroid.showWithGravity(response.data.error, ToastAndroid.LONG, ToastAndroid.TOP)
        }
      } catch (error) {
        ToastAndroid.showWithGravity(response.data.error, ToastAndroid.LONG, ToastAndroid.TOP)
        console.log('response got an error'),error;
      }
    }
        ...
        <View className=" flex-1 flex-col items-center justify-between">
          ...
          <View className="bg-white opacity-90 rounded-t-3xl w-full">
            <View className="p-5">
              ...
              <Text className="mt-3 mb-1 text-orange-500 font-bold">
                Username
              </Text>
              <TextInput
