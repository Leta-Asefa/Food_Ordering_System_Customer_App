...
const ConfirmOrder = ({navigation, route}) => {
  ...
  const {cart, selectedRestaurant, clearCart, setSelectedRestaurant} =
    useCartContext();
  ...
  useEffect(() => {
    ...
    const registerAnOrder = async () => {
      ...
      const eta =
        longestPreparationTime + Number(selectedRestaurant.durationValue);
      console.log(
        eta,
        longestPreparationTime,
        selectedRestaurant.durationValue,
      );
  ...
  const handlePayment = async method => {
    if (method === 'payNow') {
      const formData = {
        ...
        subAccountId: selectedRestaurant.restaurant.subAccountId,
  ...
  const handleCancel = async () => {
    const response = await axios.post(
      `http://localhost:4000/order/${orderResponse._id}/status`,
      {status: 'Cancelled'},
      {
        headers: {
          'Content-Type': 'application/json',
    ...
    <View className="flex-1 p-4 bg-gray-50">
      ...
      {isLoading ? (
        ...
        <View className="bg-white p-5 rounded-lg shadow-lg">
          <Text className="text-xs text-gray-800 font-semibold mb-2">
          ...
          <Text className="text-xs text-gray-600 mb-2">
            Customer: {authUser.user.username}
          </Text>
          <Text className="text-xs text-gray-600 mb-2">
            Delivery Address: {deliveryaddress?.address}
          </Text>

          <Text className="text-lg text-gray-800 font-semibold mt-3 mb-1">
            Items:
          </Text>
          <FlatList
            data={cart}
        ...
        <Text className="text-red-600 text-center">
          {orderResponse?.error || 'Failed to confirm order'}
        </Text>
      )}
    </View>
  );
