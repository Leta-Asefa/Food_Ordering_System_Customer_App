            ...
            <ScrollView className=''>
                {cart.map((cart, index) => {
                    return <View key={index.toString()} className='flex flex-row justify-between space-x-2 items-center p-2 bg-gray-200 mt-1 mx-2 rounded-lg'>
                        <Image source={{ uri: String(cart.item.image) }} className='w-14 h-14' />
                        <View>
                            <Text className='w-44'>{cart.item.name}</Text>
                            <Text className='w-44'>{cart.item.price} ETB</Text>
                        </View>
                        <View className='flex flex-row justify-between space-x-2'>
                            <TouchableOpacity className='bg-white p-1 rounded' onPress={() => handleMinus(cart.item._id, cart.quantity)}>
                                <FontAwesome name="minus" size={25} color="#f00" />
                            </TouchableOpacity>

                            <Text className='font-bold text-lg'>{cart.quantity} </Text>

                            <TouchableOpacity className='bg-white p-1 rounded' onPress={() => handlePlus(cart.item._id, cart.quantity)}>
