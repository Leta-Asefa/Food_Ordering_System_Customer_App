...
const DeliveryRatings = ({ deliveryPersonId, userId }) => {
  ...
  const fetchRatings = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`http://localhost:4000/delivery_person_rating/${deliveryPersonId}/all`);
      setRatings(res.data);
    } catch (error) {
  ...
  const submitReview = async () => {
    ...
    try {
      setSubmitting(true);
      await axios.post(`http://localhost:4000/delivery_person_rating/${deliveryPersonId}/add`, {
        userId,
        review: newReview,
        rating: newRating,
      });
